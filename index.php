...
if($LatestSql = $db->prepare("SELECT * FROM media WHERE active=1 ORDER BY id DESC LIMIT 0, 10")){
	$LatestSql->execute();
...
<div class="post-title"><a href="post-<?php echo $MediaId;?>-<?php echo $MediaLink;?>.php" <?php if ($MediaOpen==1){?> target="_blank" <?php }?>><h1><?php echo stripslashes($LatestRow['title']);?></h1></a></div><!--post-title-->

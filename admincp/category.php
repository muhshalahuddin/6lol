...
$delete = $_GET['delete'];
if ($delete == 'yes'){
$delete=$db->prepare("DELETE FROM categories WHERE id='$del'");
$delete->execute();

if ($delete= $db->prepare("DELETE FROM categories WHERE id='$del")) {
    $delete-> execute();
    
    $delete->close();
}

if ($deleter= $db->prepare("DELETE FROM media WHERE catid='$del")) {
   $deleter->execute();
...
<?php
	...
	*/
	$query = $db->prepare("SELECT COUNT(*) as num FROM categories");
	$query->execute();
...
<tbody>
	<?php
		...
		{
			$ncid = $row['id'];
			$CountSQL = $db->prepare("SELECT id FROM media WHERE catid='$ncid'");
			$CountSQL->execute();
$NumRows = $CountSQL->rowCount();
		?>
		<tr>

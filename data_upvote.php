...
$pid = $db->quote($_GET['id']);

if($Profile = $db->prepare("SELECT * FROM users WHERE uid='$pid'")){
	$Profile->execute();
...
if($LatestSql = $db->prepare("SELECT * FROM voteip LEFT JOIN media ON voteip.media_id=media.id WHERE voteip.uid='$pid' ORDER BY voteip.id and voteip.type=1 DESC LIMIT $start, 10")){
	$LatestSql->execute();
